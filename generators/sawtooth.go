...
func SawtoothTone(sr beep.SampleRate, freq float64) (beep.Streamer, error) {
	...
	if dt >= 1.0/2.0 {
		return nil, errors.New("brotholo sawtooth tone generator: samplerate must be at least 2 times grater then frequency")
	}
...
func SawtoothToneReversed(sr beep.SampleRate, freq float64) (beep.Streamer, error) {
	...
	if dt >= 1.0/2.0 {
		return nil, errors.New("brotholo triangle tone generator: samplerate must be at least 2 times grater then frequency")
	}
